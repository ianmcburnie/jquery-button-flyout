...
(function($,window,document,undefined){$.fn.buttonFlyout=function buttonFlyout(options){options=$.extend({focusManagement:false,sticky:false},options);return this.each(function onEach(){var $this=$(this);var $button=$this.find('> button, > a[role=button]');var $overlay=$this.find('> *:last-child');var isAnchorTag=($button.prop('tagName').toLowerCase()==='a');function openButtonFlyout(e){if($button.attr('aria-expanded')==='false'){$button.attr('aria-expanded','true');$overlay.attr('aria-hidden','false');if(options.focusManagement===true){$overlay.focusable().first().focus();}
...
$this.nextId('button-flyout');if(options.sticky===false){$overlay.focusExit().on('focusExit',closeButtonFlyout);$this.focusExit().on('focusExit',closeButtonFlyout);}
